"""
Liar's Deck Online - Game Engine
All game logic for one room: deck, dealing, playing, challenges, roulette.
Server-authoritative: clients never compute game state.
"""

import random
import threading
from dataclasses import dataclass, field

from shared.constants import (
    CARD_ACE, CARD_KING, CARD_QUEEN, CARD_JOKER,
    DECK_COMPOSITION, HAND_SIZE, ROULETTE_CHAMBERS,
    ROULETTE_DEAD, ROULETTE_SURVIVED,
    VALID_CLAIM_TYPES,
    PHASE_WAITING, PHASE_DEALING, PHASE_PLAYING,
    PHASE_CHALLENGING, PHASE_ROULETTE, PHASE_GAME_OVER,
)


@dataclass
class PlayerState:
    """Mutable per-player state inside a game."""
    player_id: str
    username: str
    hand: list = field(default_factory=list)
    bullet_position: int = 0     # 1-6, which chamber has the bullet
    pull_count: int = 0          # how many times pulled trigger this game
    
    is_alive: bool = True
    is_connected: bool = True
    is_ready: bool = False


class GameEngine:
    """
    Manages one game room's complete logic for 2-4 players.
    All methods assume caller holds room lock (or engine lock).
    """

    def __init__(self, player_info_list: list[dict]):
        self._lock = threading.Lock()

        # players
        self.players: dict[str, PlayerState] = {}
        self.player_order: list[str] = []
        
        for p in player_info_list:
            pid = p["id"]
            self.players[pid] = PlayerState(player_id=pid, username=p["name"])
            self.player_order.append(pid)

        # game state
        self.phase: str = PHASE_WAITING
        self.deck: list[str] = []
        self.discard_pile: list[str] = []
        self.table_card: str = ""          # the card type for this round
        self.current_turn_idx: int = 0     # index into player_order
        self.last_play: dict | None = None  # {player_id, cards, claimed_type}
        self.round_number: int = 0
        self.sequence: int = 0             # monotonic state update counter

        # roulette state
        self._roulette_loser_id: str | None = None

        # initialize bullet positions
        for ps in self.players.values():
            ps.bullet_position = random.randint(1, ROULETTE_CHAMBERS)

    # ── Deck ───────────────────────────────────────────────

    def create_deck(self) -> list[str]:
        """Build and shuffle a 20-card deck."""
        deck = []
        for card_type, count in DECK_COMPOSITION.items():
            deck.extend([card_type] * count)
        random.shuffle(deck)
        return deck

    def deal_cards(self):
        """Deal HAND_SIZE cards to each ALIVE player from deck."""
        self.deck = self.create_deck()
        for ps in self.players.values():
            ps.hand = []
            
        alive_players = [p for p in self.player_order if self.players[p].is_alive]
        
        # Deal cards in round-robin
        for _ in range(HAND_SIZE):
            for pid in alive_players:
                if self.deck:
                    self.players[pid].hand.append(self.deck.pop())
        # remaining deck stays
        self.discard_pile = []

    def choose_table_card(self) -> str:
        """Pick random card type for this round (ACE/KING/QUEEN)."""
        self.table_card = random.choice(VALID_CLAIM_TYPES)
        return self.table_card

    # ── Turn Management ────────────────────────────────────

    @property
    def current_turn_id(self) -> str:
        if self.phase == PHASE_ROULETTE and self._roulette_loser_id:
            return self._roulette_loser_id
        return self.player_order[self.current_turn_idx]

    def _advance_turn(self):
        """Switch to next ALIVE player's turn with cards."""
        for _ in range(len(self.player_order)):
            self.current_turn_idx = (self.current_turn_idx + 1) % len(self.player_order)
            pid = self.player_order[self.current_turn_idx]
            if self.players[pid].is_alive and len(self.players[pid].hand) > 0:
                return

    # ── Play Cards ─────────────────────────────────────────

    def play_cards(
        self, player_id: str, card_indices: list[int], claimed_type: str
    ) -> dict:
        """
        Player plays cards face-down, claiming they are claimed_type.
        """
        with self._lock:
            if self.phase != PHASE_PLAYING:
                return {"success": False, "error": "Wrong phase"}
            if player_id != self.current_turn_id:
                return {"success": False, "error": "Not your turn"}
            if claimed_type not in VALID_CLAIM_TYPES:
                return {"success": False, "error": "Invalid claim type"}
            if claimed_type != self.table_card:
                return {"success": False, "error": "Claimed type must match the table card"}

            alive_with_cards = [p for p in self.player_order if self.players[p].is_alive and len(self.players[p].hand) > 0]
            if len(alive_with_cards) == 1 and alive_with_cards[0] == player_id:
                return {"success": False, "error": "You are the last player with cards; you must call LIAR."}

            ps = self.players[player_id]

            # validate indices
            sorted_indices = sorted(set(card_indices), reverse=True)
            if len(sorted_indices) != len(card_indices):
                return {"success": False, "error": "Duplicate indices"}
            for idx in sorted_indices:
                if idx < 0 or idx >= len(ps.hand):
                    return {"success": False, "error": f"Index {idx} out of range"}

            # extract cards
            cards_played = []
            for idx in sorted_indices:
                cards_played.append(ps.hand.pop(idx))
            cards_played.reverse()

            # add to discard pile
            self.discard_pile.extend(cards_played)

            # record last play
            self.last_play = {
                "player_id": player_id,
                "cards": cards_played,
                "claimed_type": claimed_type,
                "count": len(cards_played),
            }

            self.sequence += 1

            # Check if all hands are empty now
            round_over = self.check_round_over()
            if not round_over:
                self._advance_turn()

            return {
                "success": True,
                "cards_played": cards_played,
                "count": len(cards_played),
                "claimed_type": claimed_type,
                "round_over": round_over,
            }

    # ── Challenge (Call Liar) ──────────────────────────────

    def call_liar(self, caller_id: str) -> dict:
        """
        Caller challenges the last play.
        """
        with self._lock:
            if self.phase != PHASE_PLAYING:
                return {"success": False, "error": "Wrong phase"}

            if not self.last_play:
                return {"success": False, "error": "No play to challenge"}

            target_id = self.last_play["player_id"]
            if caller_id == target_id:
                return {"success": False, "error": "Cannot call liar on yourself"}

            actual_cards = self.last_play["cards"]
            claimed_type = self.last_play["claimed_type"]
            was_lying = not self.check_cards_truth(actual_cards, claimed_type)

            if was_lying:
                loser_id = target_id       # liar loses
            else:
                loser_id = caller_id       # false accusation, caller loses

            # transition to roulette
            self.phase = PHASE_ROULETTE
            self._roulette_loser_id = loser_id
            self.sequence += 1

            return {
                "success": True,
                "was_lying": was_lying,
                "loser_id": loser_id,
                "revealed_cards": actual_cards,
                "claimed_type": claimed_type,
                "caller_id": caller_id,
                "target_id": target_id,
            }

    def check_cards_truth(self, cards: list[str], claimed_type: str) -> bool:
        for card in cards:
            if card != claimed_type and card != CARD_JOKER:
                return False
        return True

    # ── Roulette ───────────────────────────────────────────

    def pull_roulette(self, player_id: str) -> dict:
        """
        Player pulls the trigger.
        """
        with self._lock:
            if self.phase != PHASE_ROULETTE:
                return {"success": False, "error": "Wrong phase"}

            if player_id != self._roulette_loser_id:
                return {"success": False, "error": "Not your roulette turn"}

            ps = self.players[player_id]
            ps.pull_count += 1
            current_pull = ps.pull_count

            if current_pull == ps.bullet_position:
                # BANG
                ps.is_alive = False
                
                # Check if game is over (only 1 survivor)
                alive_pids = [p for p in self.player_order if self.players[p].is_alive]


                if len(alive_pids) == 1:
                    self.phase = PHASE_GAME_OVER
                else:
                    # Continue game with remaining players
                    # Find next alive player for the next turn, ignoring hand size
                    for _ in range(len(self.player_order)):
                        self.current_turn_idx = (self.current_turn_idx + 1) % len(self.player_order)
                        pid = self.player_order[self.current_turn_idx]
                        if self.players[pid].is_alive:
                            break
                    self.reset_round()
                
                self.sequence += 1
                return {
                    "success": True,
                    "result": ROULETTE_DEAD,
                    "pull_number": current_pull,
                    "bullet_position": ps.bullet_position,
                    "player_id": player_id,
                }
            else:
                # CLICK - survived, a new round starts!
                # survivor goes first next
                self.current_turn_idx = self.player_order.index(player_id)

                self.sequence += 1
                return {
                    "success": True,
                    "result": ROULETTE_SURVIVED,
                    "pull_number": current_pull,
                    "bullet_position": ps.bullet_position,
                    "player_id": player_id,
                    "round_over": True,
                }

    # ── Round Management ───────────────────────────────────

    def check_round_over(self) -> bool:
        """Round is over when all ALIVE players' hands are empty."""
        for pid in self.player_order:
            ps = self.players[pid]
            if ps.is_alive and len(ps.hand) > 0:
                return False
        return True

    def reset_round(self):
        """Reshuffle, redeal, pick new table card, increment round."""
        self.round_number += 1
        self.deal_cards()
        self.choose_table_card()
        self.last_play = None
        self._roulette_loser_id = None
        self.phase = PHASE_PLAYING
        self.sequence += 1

    # ── Game Start ─────────────────────────────────────────

    def start_game(self):
        """Initialize a new game: deal, pick table card, set phase."""
        with self._lock:
            self.round_number = 1
            self.deal_cards()
            self.choose_table_card()

            # find first alive player
            self.current_turn_idx = random.randint(0, len(self.player_order)-1)
            if not self.players[self.player_order[self.current_turn_idx]].is_alive:
                self._advance_turn()
                
            self.phase = PHASE_PLAYING
            self.sequence += 1

    # ── State Serialization ────────────────────────────────

    def build_state_for_player(self, player_id: str) -> dict:
        """
        Build complete game state dict for one player.
        """
        with self._lock:
            ps = self.players[player_id]

            state = {
                "game_phase": self.phase,
                "round_number": self.round_number,
                "table_card": self.table_card,
                "current_turn_player_id": self.current_turn_id,
                "sequence": self.sequence,
                "your_hand": list(ps.hand),
                "your_pull_count": ps.pull_count,
                "your_alive": ps.is_alive,
                "center_pile_count": len(self.discard_pile),
                "players": [
                    {
                        "id": p.player_id,
                        "username": p.username,
                        "alive": p.is_alive,
                        "pull_count": p.pull_count,
                        "hand_count": len(p.hand),
                        "hand": list(p.hand) if not ps.is_alive else None,
                        "connected": p.is_connected,
                    }
                    for p in self.players.values()
                ],
                "deck_remaining": len(self.deck),
            }

            if self.last_play:
                state["last_play"] = {
                    "player_id": self.last_play["player_id"],
                    "claimed_type": self.last_play["claimed_type"],
                    "count": self.last_play["count"],
                }

            if self.phase == PHASE_ROULETTE and self._roulette_loser_id:
                state["roulette_state"] = {
                    "target_player_id": self._roulette_loser_id,
                    "pull_number": self.players[self._roulette_loser_id].pull_count + 1,
                    "chamber_count": ROULETTE_CHAMBERS,
                }

            return state

    def get_winner(self) -> str | None:
        """Return winner player_id if game is over (only 1 survivor)."""
        alive_pids = [pid for pid, ps in self.players.items() if ps.is_alive]
        if len(alive_pids) == 1:
            return alive_pids[0]
        return None

    def get_loser(self) -> str | None:
        """In N-player game, loser is the one who just died."""
        for ps in self.players.values():
            if not ps.is_alive:
                return ps.player_id
        return None

    def set_player_connected(self, player_id: str, connected: bool):
        with self._lock:
            if player_id in self.players:
                self.players[player_id].is_connected = connected

    def forfeit(self, player_id: str):
        """Player forfeits. Mark dead."""
        with self._lock:
            if player_id in self.players:
                self.players[player_id].is_alive = False
                # If only 1 survivor left, game over
                alive_pids = [p for p in self.player_order if self.players[p].is_alive]
                if len(alive_pids) == 1:
                    self.phase = PHASE_GAME_OVER
                self.sequence += 1
